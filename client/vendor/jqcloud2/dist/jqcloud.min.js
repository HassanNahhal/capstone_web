...
! function (a, b) {
    ...
    c.DEFAULTS = {
        ...
        hitTest: function (a) {
            for (var b = 0, c = this.data.placed_words.length; c > b; b++)
        ...
        drawWordCloud: function () {
            ...
            if (this.$element.children('[id^="' + this.data.namespace + '"]').remove(), 0 !== this.word_array.length) {
                for (a = 0, b = this.word_array.length; b > a; a++) this.word_array[a].weight = parseFloat(this.word_array[a].weight, 10);
                ...
                if (this.options.delay > 0) this.drawOneWordDelayed();
                    ...
                    for (a = 0, b = this.word_array.length; b > a; a++) this.drawOneWord(a, this.word_array[a]);
        ...
        drawOneWord: function (b, c) {
            ...
            for (c.attr = a.extend({}, c.html, {
                    id: g
                }), this.data.max_weight != this.data.min_weight && (l = Math.round(1 * (c.weight - this.data.min_weight) * (this.options.steps - 1) / (this.data.max_weight - this.data.min_weight)) + 1), d = a("<span>").attr(c.attr), this.options.classPattern && d.addClass(this.options.classPattern.replace("{n}", l)), this.data.colors.length && d.css("color", this.data.colors[l - 1]), this.data.sizes.length && d.css("font-size", this.data.sizes[l - 1]), this.options.template ? d.html(this.options.template(c)) : c.link ? ("string" == typeof c.link && (c.link = {
                    href: c.link
        ...
        destroy: function () {
            this.clearTimeouts(), this.$element.removeClass("jqcloud"), this.$element.removeData("jqcloud"), this.$element.children('[id^="' + this.data.namespace + '"]').remove()
        },
        update: function (a) {
